...
def _initialize_cli(
    root: Annotated[
        Path,
        typer.Option(
            help="The project root directory.",dir_okay=True, writable=True, resolve_path=True,
            autocompletion=path_autocomplete(file_okay=False, dir_okay=True, writable=True, match_wildcard="*"),
        ),
    ],
    force: Annotated[
        bool,typer.Option(help="Force initialization even if the project already exists."),
    ] = False,
):
...
def _index_cli(
    ...
    root: Annotated[
        Path,
        typer.Option(
            help="The project root directory.", exists=True, dir_okay=True, writable=True, resolve_path=True,
            autocompletion=path_autocomplete(file_okay=False, dir_okay=True, writable=True, match_wildcard="*"),
        ),
    ...
    verbose: Annotated[
        bool,
        typer.Option(help="Run the indexing pipeline with verbose logging")
    ] = False,
    memprofile: Annotated[
        bool,
        typer.Option(help="Run the indexing pipeline with memory profiling")
    ] = False,
    logger: Annotated[
    ...
    dry_run: Annotated[
        bool,
        typer.Option(help="Run the indexing pipeline without executing any steps to inspect and validate the configuration."),
    ] = False,
...
def _update_cli(
    ...
    root: Annotated[
        Path,
        typer.Option(help="The project root directory.", exists=True, dir_okay=True, writable=True, resolve_path=True,),
    ] = Path(),  # set default to current directory
    method: Annotated[
        IndexingMethod, typer.Option(help="The indexing method to use.")
    ] = IndexingMethod.Standard,
    ...
    output: Annotated[
        Path | None,
        typer.Option(help="Indexing pipeline output directory. Overrides output.base_dir in the configuration file.",dir_okay=True, writable=True, resolve_path=True,),
    ] = None,
):
    """
...
def _prompt_tune_cli(
    ...
    domain: Annotated[
        str | None,
        typer.Option(
            help="The domain your input data is related to. For example 'space science', 'microbiology', "
                 "'environmental news'. If not defined, a domain will be inferred from the input data."
        ),
    ] = None,
    ...
    chunk_size: Annotated[
        int,
        typer.Option(help="The size of each example text chunk. Overrides chunks.size in the configuration file."),
    ] = graphrag_config_defaults.chunks.size,
    overlap: Annotated[
        int,
        typer.Option(help="The overlap size for chunking documents. Overrides chunks.overlap in the configuration file"),
    ...
    output: Annotated[
        Path,
        typer.Option(help="The directory to save prompts to, relative to the project root directory.",
                    dir_okay=True, writable=True, resolve_path=True,),
    ] = Path("prompts"),
):
...
def _query_cli(
    ...
    community_level: Annotated[
        int,
        typer.Option(help="The community level in the Leiden community hierarchy from which to load community reports. " \
                          "Higher values represent reports from smaller communities."),
    ] = 2,
    ...
    response_type: Annotated[
        str,
        typer.Option(help="Free form text describing the response type and format, can be anything, e.g. Multiple Paragraphs, " \
                          "Single Paragraph, Single Sentence, List of 3-7 Points, Single Page, Multi-Page Report. Default: Multiple Paragraphs"),
    ] = "Multiple Paragraphs",
    streaming: Annotated[
        bool,
        typer.Option(help="Print response in a streaming manner.")
    ] = False,
):
